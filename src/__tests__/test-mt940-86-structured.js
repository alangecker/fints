...
const testData = [
    {
        str: "105?00FOLGELASTSCHRIFT?109248?20EREF+1003760276550 PP.7723.?21PP PAYPAL?22MREF+5Z7J224S5B3QS?23CRED+LU96ZZZ000000000000000?240058?25SVWZ+PP.7723.PP . GOOGLE, I?26hr Einkauf bei GOOGLE?30DEUTDEFFXXX?31DE88500700100175526303?32PayPal (Europe) S.a.r.l. et?33 Cie., S.C.A.?34992",
        parsed: {
            bic: "DEUTDEFFXXX",
            ...
            reference: {
                ...
                raw: "EREF+1003760276550 PP.7723.PP PAYPALMREF+5Z7J224S5B3QSCRED+LU96ZZZ0000000000000000058SVWZ+PP.7723.PP . GOOGLE, Ihr Einkauf bei GOOGLE",
            },
            primaNota: "9248",
    ...
    {
        str: "177?00ONLINE-UEBERWEISUNG?109310?20KREF+1053050069-20180829182?21432?22SVWZ+KMC Huellen?23DATUM 29.08.2018, 18.25 UHR?241.TAN 389252?30COBADEFFXXX?31DE32250400660167600600?32Players-Point GbR?34997",
        parsed: {
            bic: "COBADEFFXXX",
...
describe("parse86Structured", () => {
    testData.forEach(({ str, parsed }, index) => {
        test(`Parsing the 86 section of MT940 data, test data set ${index}`, () => {
            expect(mt940_86_structured_1.parse86Structured(str)).toEqual(parsed);
        });
    });
});
describe("is86Structured", () => {
    testData.forEach(({ str }, index) => {
        test(`detectes test data set ${index} as valid`, () => {
            expect(mt940_86_structured_1.is86Structured(str)).toBe(true);
        });
    });
    [
        "",
        ...
        test(`detects "${str}" as invalid`, () => {
            expect(mt940_86_structured_1.is86Structured(str)).toBe(false);
        });
    });
});
//# sourceMappingURL=test-mt940-86-structured.js.map